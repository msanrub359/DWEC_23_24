"use strict";
import { addClientes, getClientes, deleteCliente, getCliente, updateCliente } from "./API.js";
...
const mostrarClientes=async()=>{
  ...
  //borrar tbody
  $('.table tbody').empty();
  if(clientes.data.length>0){ //cargar clientes
...
const borrarCliente=function(){
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      mensaje(`Cliente ${texto.mensaje}`, 'success')
...
const modificarCliente=async()=>{
  ...
  const cliente={
   ...
   console.log(cliente);
   const datos= await updateCliente(cliente, id);
  ...
  if (datos.mensaje=="actualizado"){
    mensaje("Cliente modificado", "success");
    mostrarClientes();
  }else{
    mensaje("Cliente no modificado", "error");
  }
